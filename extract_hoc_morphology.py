...
def dend_area(hoc_lines, dlist, dbe): #returns a list of dendrites' lengths
	...
	for dend in dlist:
		...
		for i in range(b,e):
			if re.search(r'pt3dadd\((.*?),(.*?),(.*?),(.*?)\)', hoc_lines[i]):
				...
				y=float(regex.group(2))
				z=float(regex.group(3))
				diam=float(regex.group(3))

				if i != b:
					dist_sum.append(distance(x,xp,y,yp,z,zp))
					a=2*pi*diam+2*pi*(d**2)
		sum_area=sum(dist_sum)
		area[dend]=sum_area
		print dend, area[dend]
